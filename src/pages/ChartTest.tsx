...
type FanChartNode = { family: topola.JsonFam | undefined, father: topola.JsonIndi | undefined, mother: topola.JsonIndi | undefined };
type FanChartLevel = Array<FanChartNode>;
type FanChartDescententLevel = Array<{ person: topola.JsonIndi | undefined, families: Array<{ family: topola.JsonFam | undefined, ratio: number }>, ratio: number }>;
type FanChartData = {
...
type FanChartOptions = {
  nameRenderer: (person: topola.JsonIndi) => { text: string | undefined, subText: string | undefined };
...
class FanChart {
  ...
  constructor(svg: d3.Selection<SVGGElement, unknown, null, undefined>, width: number, data: FanChartData, options: Partial<FanChartOptions> = {}) {
    this.options = {
      nameRenderer: (person: topola.JsonIndi) => ({ text: `${person.firstName}, ${person.lastName}`, subText: `${person.birth?.date?.year || ''} - ${person.death?.date?.year || ''}` }),
  ...
  private adjustFontSize(
    textElement: d3.Selection<SVGTextElement, unknown, null, undefined> | d3.Selection<SVGTextPathElement, unknown, null, undefined>,
    ...
    let textFontSize = Math.min(defaultFontSize, (maxTextLength / textLength) * 16); // Adjust the base font size as needed
  ...
  private addText(
    ...
    if (direction === 'radial') {
      ...
      const radialTextArcAngle = startAngle + (endAngle - startAngle) * placement;
      const radialTextArc = d3.arc()
      ...
      const textRadius = innnerRadius + (outerRadius - innnerRadius) * placement;
  ...
  private addFamilyArc(level: number, i: number, text: string | undefined = undefined) {
    ...
    const innerRadius = (this.simpleArcHeight + this.familyArcHeight + this.padding) * Math.min(level, this.highArcMinLevel)
      + this.simpleArcHeight * 0.5 + this.padding
      + Math.max(0, level - this.highArcMinLevel) * (this.highArcHeight + this.familyArcHeight + this.padding);
    ...
    if (text !== undefined) {
      ...
      this.adjustFontSize(textElement, this.familyArcHeight * 0.7, maxTextLength);
    }
    
  }

  private addPersonArc(level: number, i: number, text1: string | undefined, text2: string | undefined = undefined) {
    ...
    this.svg.append("path")
      ...
      .attr("class", `personArc personArcLevel${level} personArcIdx${i} personArcInCoupleIdx${i % 2}`);
    
    // TODO generate better id
    if (text1 !== undefined) {
      const textDirection = level < this.highArcMinLevel ? 'circumferential' : 'radial'
      const text1El = this.addText(arcG, 0.6, 16, text1, `person-text1-${level}-${i}`, textDirection);
      const text1FontSize = Number.parseInt(text1El.style("font-size").replace("px", ""));
      if (text2 !== undefined) {
        this.addText(arcG, 0.3, text1FontSize * 0.8, text2, `person-text2-${level}-${i}`, textDirection);
      }
    }
  }
  ...
  public addDescendentArc(
    ...
    const startAngle = (this.full_arc / sectionsOfLevel) * startSection - this.full_arc / 2 + Math.PI;
    const endAngle = (this.full_arc / sectionsOfLevel) * (startSection + size) - this.full_arc / 2 + Math.PI;

    const innerRadius = (this.simpleArcHeight + this.familyArcHeight + this.padding) * (highArcMinLevel !== undefined ? Math.min(level, highArcMinLevel) : level)
      + this.simpleArcHeight * 0.5 + this.familyArcHeight + this.padding
      + (highArcMinLevel !== undefined
        ? (this.highArcHeight + this.familyArcHeight + this.padding) * Math.max(0, level - highArcMinLevel)
      : 0
    );
    
    const outerRadius = innerRadius + (highArcMinLevel === undefined || level < highArcMinLevel ? this.simpleArcHeight : this.highArcHeight);
    ...
    this.svg.append("path")
      .attr("id", pathId)
      .attr("d", arcG as unknown as string)
      .attr("class", `personArc descendantPersonArc descendantArcLevel${level}`);
    
    // TODO generate better id
    if (text !== undefined) {
      const textDirection = highArcMinLevel ? 'circumferential' : 'radial'
      const text1El = this.addText(arcG, 0.6, 16, text, `descendant-text1-${level}-${startSection}`, textDirection);
    }
  ...
  public render() {
    ...
    for (let l = 0; l < this.num_of_levels; l++) {
      for (let familyIdx = 0; familyIdx < 2 ** l; familyIdx++) {
        ...
        if (node !== undefined && node.family !== undefined) {
          this.addFamilyArc(l, familyIdx, this.options.familyRenderer(node.family));
          if (node?.father !== undefined) {
            const fatherName = this.options.nameRenderer(node.father);
            this.addPersonArc(l, familyIdx * 2, fatherName.text, fatherName.subText);
          }
          if (node?.mother !== undefined) {
            const motherName = this.options.nameRenderer(node.mother);
            this.addPersonArc(l, familyIdx * 2 + 1, motherName.text, motherName.subText);
          }
        }
      }
    }
    ...
    for (let l = 0; l < this.num_of_levels; l++) {
      const num_of_sections = this.data.down[l].reduce((acc, person) => acc + person.ratio, 0);
      if (highArcMinLevel === undefined) {
        const min_ratio = Math.min(...this.data.down[l].map(person => person.ratio / num_of_sections));
        if (min_ratio < 1 / 8) {
          highArcMinLevel = l;
        }
      }
      ...
      for (let person of this.data.down[l]) {
        if (person.person !== undefined) {
          this.addDescendentArc(l, num_of_sections, current_section, person.ratio, highArcMinLevel, this.options.nameRenderer(person.person).text);
        }
...
function toFamilyTreeAncestors(
  data: { indiMap: Map<string, topola.JsonIndi>, famMap: Map<String, topola.JsonFam> },
  ...
  if (maxLevels > 0) {
    const fatherSubTree = toFamilyTreeAncestors(data, father?.famc, maxLevels - 1);
    const motherSubTree = toFamilyTreeAncestors(data, mother?.famc, maxLevels - 1);
...
function toFamilyTreeDescendants(data: { indiMap: Map<string, topola.JsonIndi>, famMap: Map<String, topola.JsonFam> }, famId: string | undefined, maxLevels: number): Array<FanChartDescententLevel> {
  ...
  const children = (family?.children?.length ? family.children : [undefined]).map(childId => childId !== undefined ? data.indiMap.get(childId) : undefined)
    .map(child => {
      if (child === undefined) {
        return {
          person: undefined,
          families: [undefined],
          subLevelPerFamily: [toFamilyTreeDescendants(data, undefined, maxLevels - 1)],
          ratio: 1,
        };
      }
      const families = child?.fams?.map(famId => data.famMap.get(famId)).filter(f => f !== undefined) as Array<topola.JsonFam>;

      const subLevelPerFamily = (child?.fams?.length ? child.fams : [undefined]).map(famId => {
        return toFamilyTreeDescendants(data, famId, maxLevels - 1);
      });

      const ratio = subLevelPerFamily.reduce((acc, subLevels) => acc + subLevels[0].reduce((acc, subLevel) => acc + subLevel.ratio, 0), 0);
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    for (let subLevels of child.subLevelPerFamily) {
      for (let j = 0; j < subLevels.length; j++) {
        subLevelsAccumulator[j] = [ ...(subLevelsAccumulator[j] || []), ...subLevels[j] ];
      };
    }
    ...
    thisLevel.push({
      person: child.person,
      families: child.families.map((family, i) => ({ family, ratio: childSubLevels[i].reduce((acc, subLevel) => acc + subLevel.ratio, 0) })),
      ratio: child.ratio,
    });
  }
...
function toFamilyTreeData(data: topola.JsonGedcomData, famId: string | undefined, maxLevels: number): FanChartData {
