...
import { Person } from '../../src/gql/graphql.js'

interface ColorFunctionBase {
  type?: string,
  fct: (person: Person) => unknown | undefined
}

interface NumberColorFunction extends ColorFunctionBase {
  type: 'number'
  fct: (person: Person) => number | undefined
}

interface CountColorFunction extends ColorFunctionBase {
  type: 'count'
  fct: (person: Person) => number | undefined
}


type ColorFunction = ColorFunctionBase | NumberColorFunction


const colorFunctions: Record<string, ColorFunction> = {
  default: {
    fct: d => {
      if (d.depth === 0) {
...
function mapPersons<T>(person: Person | null | undefined, callback: (person: Person) => T): T[] {
  return (person ? [callback(person)] : []).concat(
    (person?.families?.reduce((acc, fam) => return [
      ...acc,
      ...fam.children,
    ], []).reduce(
...
export function FanChart(
  ...
  if (colorFunctionInfo.type === 'number') {
    const numberFunction = colorFunctionInfo.fct as NumberColorFunction['fct']
